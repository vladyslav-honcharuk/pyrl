...
Usage:
    python3 train_kappa_all_parallel.py models/gambling.py
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 train_kappa_all_parallel.py <model_file>")
        print("Example: python3 train_kappa_all_parallel.py models/gambling.py")
        sys.exit(1)
    ...
    for kappa, suffix, path, status in all_results:
        if "✓" in status:
            display_path = path.replace(f"work/data/{base_name}/", "") if path else ""
            if status == "✓ BASE":
