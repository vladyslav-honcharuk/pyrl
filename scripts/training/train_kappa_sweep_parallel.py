...
Usage:
    python3 scripts/training/train_kappa_sweep_parallel.py models/gambling.py
"""
...
def train_chain(chain_name, kappa_values, modelfile, base_name, finetune_iter, result_queue, log_queue):
    ...
    for i, kappa in enumerate(kappa_values, 1):
        ...
        if prev_suffix:
            ...
            pretrained_path = os.path.join(REPO_ROOT, 'data', 'weights', prev_name, f'{prev_name}.pkl')
        else:
            pretrained_path = os.path.join(REPO_ROOT, 'data', 'weights', base_name, f'{base_name}.pkl')
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 scripts/training/train_kappa_sweep_parallel.py <model_file>")
        print("Example: python3 scripts/training/train_kappa_sweep_parallel.py models/gambling.py")
        sys.exit(1)
    ...
    for kappa, suffix, path, status in all_results:
        if "✓" in status:
            display_path = path.replace(os.path.join(REPO_ROOT, 'data', 'weights', base_name, ''), "")
